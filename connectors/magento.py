...
DEFAULT_PRODUCT_FIELDS = (
    "items[sku,name,price,attribute_set_id,created_at,extension_attributes[stock_item[qty,is_in_stock]]],total_count"
)
...
class Client:
    ...
    def get_default_products(self, qty_min=1, page_size=200):
        self._ensure()
        items = list(
            self._cli.iter_products_qty_gt(qty_min=qty_min, page_size=page_size)
        )
        return {"items": items}
